...
def main(args, override_args=None):
    ...
    for subset in args.valid_subset.split(','):
        ...
        for i, sample in enumerate(progress):
            ...
            res_samples.append((sample['id'][0].item(), src_str, target_str, log_output['nll_loss'].item()))
        ...
        if len(args.gen_output) > 0:
            res_samples = sorted(res_samples, key=lambda x: x[0])
            save_lines(args.gen_output + '.%s.score' % subset, ['\t'.join([str(v) for v in line]) for line in res_samples])
