...
# Usage:
# e.g.
# bash run-doc.sh data exp_test iwslt17
# bash run-doc.sh train exp_test iwslt17
# bash run-doc.sh test exp_test iwslt17
...
if [ $mode == "data" ]; then
  ...
  bash prepare-doc.sh $data $exp_path
  python -m fairseq_cli.preprocess --task translation --joined-dictionary --source-lang $slang --target-lang $tlang \
         --trainpref $seg_path/train --validpref $seg_path/valid --testpref $seg_path/test --destdir $bin_path --workers 8
