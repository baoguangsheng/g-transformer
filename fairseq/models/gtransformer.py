...
class GTransformerModel(FairseqEncoderDecoderModel):
    """
    ...
    def build_encoder(cls, args, src_dict, embed_tokens):
        return GTransformerEncoder(args, src_dict, embed_tokens)

    @classmethod
    def build_decoder(cls, args, tgt_dict, embed_tokens):
        return GTransformerDecoder(
            args,
            tgt_dict,
            embed_tokens,
...
class GTransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        self.eod = '[{}]'.format(args.source_lang)
        self.partial_mode = args.doc_mode == 'partial'
        ...
        start_ctxlayer = args.encoder_layers - args.encoder_ctxlayers
        self.layers.extend(
            [self.build_encoder_layer(args, add_global_attn=self.partial_mode and i >= start_ctxlayer)
    ...
    def build_encoder_layer(self, args, add_global_attn):
        return GTransformerEncoderLayer(args, add_global_attn)
    ...
    def forward(self, src_tokens, src_lengths, return_all_hiddens: bool = False):
        ...
        """
        x, embedding, src_tags = self.forward_embedding(src_tokens)
        ...
        for layer in self.layers:
            x, layer_attn = layer(x, padding_mask, local_attn_mask=local_attn_mask)
    ...
    def reorder_encoder_out(self, encoder_out: EncoderOut, new_order):
        ...
        """
        encoder_tags: Optional[Tensor] = encoder_out.encoder_tags
        ...
        new_encoder_tags = (
            encoder_tags
            if encoder_tags is None
            else encoder_tags.index_select(0, new_order)
        )
...
class GTransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        self.eod = '[{}]'.format(args.target_lang)
        self.self_partial_mode = args.doc_mode == 'partial'
        self.cross_partial_mode = args.doc_mode == 'partial'
        self.dropout = args.dropout
        ...
        if self.decoder_layerdrop > 0.0:
            ...
            self.layers = nn.ModuleList([])
        start_dec_ctxlayer = args.decoder_layers - args.decoder_ctxlayers
        start_crs_ctxlayer = args.decoder_layers - args.cross_ctxlayers
        self.layers.extend(
            [
                self.build_decoder_layer(args, no_encoder_attn,
                                         self.self_partial_mode and i >= start_dec_ctxlayer,
                                         self.cross_partial_mode and i >= start_crs_ctxlayer)
    ...
    def build_decoder_layer(self, args, no_encoder_attn, dec_add_global_attn, crs_add_global_attn):
        return GTransformerDecoderLayer(args, no_encoder_attn, dec_add_global_attn, crs_add_global_attn)
...
@register_model_architecture("gtransformer", "gtransformer_tiny")
def gtransformer_tiny(args):
...
@register_model_architecture("gtransformer", "gtransformer_small")
def gtransformer_small(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 1024)
    args.encoder_attention_heads = getattr(args, "encoder_attention_heads", 4)
    args.encoder_layers = getattr(args, "encoder_layers", 6)
...
@register_model_architecture("gtransformer", "gtransformer_base")
def gtransformer_base(args):
    args.encoder_layers = getattr(args, "encoder_layers", 6)
    args.encoder_attention_heads = getattr(args, "encoder_attention_heads", 8)
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 2048)
...
@register_model_architecture("gtransformer", "gtransformer_large")
def gtransformer_large(args):
