...
class DocSampleDataset(FairseqDataset):
    def __init__(self, dataset, seed, randsample):
        ...
        idx = idx[:self.randsample]
        idx = np.sort(idx)
        self.indices = indices[idx]
        logger.info('DocDataset: random selected %s samples.' % len(self.indices))
    ...
    def prefetch(self, indices):
        self.dataset.prefetch([self.indices[index] for index in indices])
