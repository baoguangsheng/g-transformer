...
class Trainer(object):
    ...
    def optimizer(self):
        if self._optimizer is None:
            self._build_default_optimizer()
        return self._optimizer

    @property
    def lr_scheduler(self):
        if self._lr_scheduler is None:
            self._build_default_optimizer()  # this will initialize self._lr_scheduler
        return self._lr_scheduler

    def _build_optimizer(self, args, params):
        if args.fp16 or args.bf16:
            assert type(params[0]) == torch.nn.parameter.Parameter, "fp16 does not support param groups."
            ...
            if args.memory_efficient_fp16 or args.memory_efficient_bf16:
    ...
    # Guangsheng Bao: separate learning-rate for pretrained and random-initialized parameters.
    def _build_partial_optimizer(self, missing_keys):
        def _pure_name(param_name):
            if param_name.startswith('module.'):
                return param_name[len('module.'):]
            return param_name

        assert all(key.find('_global.') > 0 or key.find('_gate.') > 0 for key in missing_keys)

        params_randinit = list(
            v[1] for v in
            filter(
                lambda p: p[1].requires_grad and _pure_name(p[0]) in missing_keys,
        ...
        params_loaded = list(
            v[1] for v in
            filter(
                lambda p: p[1].requires_grad and _pure_name(p[0]) not in missing_keys,
                chain(self.model.named_parameters(), self.criterion.named_parameters()),
            )
        )
        params = [{'params': params_randinit, 'weight_decay': getattr(self.args, 'weight_decay_randinit', 0)},
                  {'params': params_loaded, 'lr_scale': getattr(self.args, 'lr_scale_pretrained', 1)}]
        self._optimizer, self._lr_scheduler = self._build_optimizer(self.args, params)
        logger.warning('Build partial optimizer: randinit=%s, loaded=%s(lr_scale=%s)'
                       % (len(params[0]['params']), len(params[1]['params']), params[1]['lr_scale']))
    ...
    def load_checkpoint(
        ...
        if bexists:
            ...
            try:
                ...
                if getattr(self.args, "load_partial_global_from_local", False):
                    state_model["doc_attn_global_from_attn_local"] = True
                res = self.get_model().load_state_dict(
                    state_model, strict=strict, args=self.args
                )
                logger.warning('Load checkpoint: missing_keys=%s' % res.missing_keys)
                logger.warning('Load checkpoint: unexpected_keys=%s' % res.unexpected_keys)
            ...
            # Guangsheng Bao: if there are som unexpected keys, raise exception
            key_global = "doc_attn_global_from_attn_local"
            if key_global in res.unexpected_keys:
                res.unexpected_keys.remove(key_global)

            if len(res.unexpected_keys) > 0:
                raise Exception('Unexpected keys: %s' % res.unexpected_keys)

            # Guangsheng Bao: if there are some missing keys,
            if len(res.missing_keys) > 0 and self.args.doc_double_lr:
                self._build_partial_optimizer(res.missing_keys)
