...
class SacrebleuScorer(object):
    ...
    def precision(self, idx):
        return self.sacrebleu.corpus_bleu(self.sys, [self.ref]).precisions[idx]
    ...
    def result_string(self, order=4):
        if order != 4:
            raise NotImplementedError
        res = self.sacrebleu.corpus_bleu(self.sys, [self.ref]).format()
        # to keep consistent with other scorers
        parts = res.split()
        part1 = ' '.join(parts[:3])  # BLEU = 22.46
        part2 = parts[3]
        part3 = ' '.join(parts[4:]).replace(' = ', '=').replace(' ', ', ')
