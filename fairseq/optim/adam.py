...
class Adam(torch.optim.Optimizer):
    ...
    def step(self, closure=None):
        ...
        """
        def _scaled_lr(group):
            lr_scale = group['lr_scale'] if 'lr_scale' in group else 1
            return group['lr'] * lr_scale
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                step_size = _scaled_lr(group) * math.sqrt(bias_correction2) / bias_correction1

                if group['weight_decay'] != 0:
                    p_data_fp32.add_(p_data_fp32, alpha=-group['weight_decay'] * _scaled_lr(group))
